...
def CheckingBack(txdiff,LimitGaz): 
         i = 0.085*LimitGaz # 95% of percentile 75% (Difference MaxPriority TargetedTX BackRuningTx)
         cou=0
         while txdiff >= 0:
            txdiff -= i 
            cou+=1 
...
with st.form("estimate_MaxPriorityFBRtx_form"):
  ...
  if submitted:
    ...
    if (difBack/int(limit_gas))<-1: # check if diff in GasPrice of Back Runing Tx and TargetedTx is less than -1 Gwei, to handle a huge gap between Tx
         BackRuningTxMaxFee=BackRuningTxMaxFee-difBack-0.085*int(limit_gas)
         BackPriorityFeeperUnitGaz=PredictedBack.reshape(-1)[0]-difBack/int(limit_gas)-0.085
